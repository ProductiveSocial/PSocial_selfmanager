...
class HabitService : HabitRepository {
    override suspend fun getHabitsByUserId(userId: Long): PaginatedResponse<HabitResponse> =
        query {
            val habits =
                HabitDAO.Companion.find { HabitTable.userId eq userId }
                    .map { it.toResponse() }
            PaginatedResponse(
                data = habits,
    ...
    override suspend fun getHabitById(userId: Long, habitId: Long): HabitResponse? =
        query {
            HabitDAO.Companion.find { (HabitTable.userId eq userId) and (HabitTable.id eq habitId) }
                .singleOrNull()?.toResponse()
        }

    override suspend fun addHabit(userId: Long, habit: HabitRequest): HabitResponse =
        query {
            validateUserAndProject(userId, habit.projectId)

            val newHabit =
                HabitDAO.Companion.new {
    ...
    override suspend fun updateHabit(userId: Long, habitId: Long, habit: UpdateHabitRequest): HabitResponse =
        query {
            ...
            habit.projectId?.let {
                ...
                existingHabit.projectId = EntityID(
                    it,
                    ProjectTable
                )
            }
    ...
    override suspend fun deleteHabit(userId: Long, habitId: Long): HabitResponse =
        query {
            val habit =
                HabitDAO.Companion.find { (HabitTable.userId eq userId) and (HabitTable.id eq habitId) }
                    .singleOrNull() ?: throw NotFoundException("Habit not found")
...
fun deleteHabitCascade(habit: HabitDAO, recordTombstone: Boolean = false) {
    ...
    habit.tags = SizedCollection(emptyList())
    habit.delete()
}
